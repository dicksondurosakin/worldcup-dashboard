...
function App() {
  ...
  return <div className={dark ? 'app dark' : 'app'}>
    ...
    <section className="toolbar">
      ...
      <select value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">All groups</option>{Object.keys(groups).map(g => <option key={g} value={g}>Group {g}</option>)}</select>
      <button className="pill" onClick={()=>setFilter('faves')}>⭐ Favourites</button>
    </section>
    <main className="layout">
      ...
      <aside className="panel">
        ...
        <div className="fixtures">{teamMatches.map(match => <Match key={match.id} match={match}/>)}</div>
      </aside>
    </main>
  </div>;
}
